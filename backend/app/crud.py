...
def create_game(db: Session, game_date: date, player_ids: List[int], payer_id: int=None, fetcher_id: int=None):
    ...
    # Grant immunity to payer (if applicable) and remove it permanently if used
    if payer_id:
        payer = db.query(models.Player).filter(models.Player.id == payer_id).first()
        if payer and payer.has_immunity:
            payer.has_immunity = False  # Permanent loss of immunity
            db.commit()

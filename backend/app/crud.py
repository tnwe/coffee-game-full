"""
CRUD operations for backward compatibility
"""
from sqlalchemy.orm import Session
from .models import Player, Game, GamePlayer, WeekState
...
def create_game(db: Session, game_date: date, player_ids: List[int], payer_id: int=None, fetcher_id: int=None):
    g = Game(date=game_date, payer_id=payer_id, fetcher_id=fetcher_id)
    db.add(g)
    db.commit()
    db.refresh(g)
    # add participations
    for pid in player_ids:
        gp = GamePlayer(game_id=g.id, player_id=pid)
        db.add(gp)
    db.commit()
    return g
...
def list_games(db: Session):
    ...
    out = []
    for g in games:
        participants = [gp.player_id for gp in db.query(GamePlayer).filter(GamePlayer.game_id==g.id).all()]
        out.append({
            "id": g.id,
